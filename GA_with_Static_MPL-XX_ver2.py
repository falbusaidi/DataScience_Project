...
@author: fahad
@purpose:  the purpose of this code is to use a genetic algorithm to determine best featuere subset for a static ANN 
...
# Glocal variables initialise from the command line 
script = sys.argv[0]
file = int(sys.argv[1])
unit = int(sys.argv[2])
...
def build_model(inputnum):
    ...
    #layer_1 = int((inputnum+1)*(2/3))
    #layer_2 = int((inputnum+layer_1+1)*(2/3))   
    layer_1= 12
    model = Sequential()
    model.add(Dense(units= layer_1, kernel_initializer = 'uniform', activation = 'relu', input_dim = inputnum))
    model.add(Dense(units= layer_1, kernel_initializer = 'uniform', activation = 'relu'))
...
def getFitness(individual, X_train, X_test, y_train, y_test, scale):
    ...
    # determine the coloumns which need to be dropped 
    ...
    # build the ANN model and fit using the training data 
    ...
    # Return calculated RMSE as fitness
    return (accuracy,)
